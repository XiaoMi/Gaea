...
| 字段名称                      | 字段类型          | 字段含义                                                                 |
|-------------------------------|-------------------|--------------------------------------------------------------------------|
| name                          | string            | 命名空间唯一标识符                                                       |
| online                        | bool              | 是否在线（逻辑上下线开关）                                               |
| read_only                     | bool              | 是否只读                                                                 |
| allowed_dbs                   | map[string]bool   | 允许访问的虚拟库白名单                                                   |
| default_phy_dbs               | map[string]string | 虚拟库默认对应物理库映射                                                 |
| slow_sql_time                 | string            | 慢SQL阈值（单位ms）                                                      |
| black_sql                     | []string          | SQL黑名单（MD5列表）                                                     |
| allowed_ip                    | []string          | IP白名单（CIDR格式）                                                     |
| slices                        | []*Slice          | 物理实例组配置                                                           |
| shard_rules                   | []*Shard          | 分片规则配置                                                             |
| users                         | []*User           | 客户端用户配置                                                           |
| global_sequences              | []*GlobalSequence | 全局序列生成器配置                                                       |
| default_slice                 | string            | 默认访问分片                                                             |
| open_general_log              | bool              | **（已废弃）** 通用日志开关                                              |
| is_encrypt                    | bool              | 是否加密存储敏感数据                                                     |
| max_sql_execute_time          | int               | SQL最大执行时间（秒），0=不限制                                          |
| max_sql_result_size           | int               | 单分片结果集限制（默认10000，-1=不限制）                                 |
| max_client_connections        | int               | 前端最大连接数限制                                                       |
| down_after_no_alive           | int               | MySQL离线标记时间（秒）                                                  |
| seconds_behind_master         | uint64            | Slave最大延迟时间（秒），0=不限制                                        |
| check_select_lock             | bool              | 强制SELECT FOR UPDATE到主库（默认true）                                  |
| local_slave_read_priority     | int               | 本地从库策略：0-关闭/1-优先本机房/2-强制本机房                           |
| support_multi_query           | bool              | 多语句执行支持（默认false）                                              |
| set_for_keep_session          | bool              | 会话绑定开关                                                             |
| client_qps_limit              | uint32            | QPS限制（0=不限制）                                                      |
| support_limit_transaction     | bool              | 事务限流控制                                                             |
| allowed_session_variables     | map[string]string | 允许设置的会话变量（gaea2.4+）                                           |
| fuse_window_size              | int               | 熔断观测窗口（秒）                                                       |
| fuse_min_error_count          | int64             | 熔断触发错误数量                                                         |
| fallback_to_master_on_slave_fail | string         | 从库故障回退："on"-回退主库/"off"-不回退                               |
| fuse_enabled                  | string            | 熔断开关："on"/"off"                                                 |
| default_charset               | string            | 默认字符集                                                               |
| default_collation             | string            | 默认排序规则                                                             |
...
| 字段名称          | 类型       | 描述与枚举值                                                                 |
|-------------------|------------|------------------------------------------------------------------------------|
| `user_name`       | `string`   | 登录用户名 (唯一标识)                                                       |
| `password`        | `string`   | 加密后的密码                                                                 |
| `namespace`       | `string`   | 资源隔离的命名空间                                                           |
| `rw_flag`         | `int`      | 读写权限标识：<br>`1` - 只读 <br>`2` - 读写                                 |
| `rw_split`        | `int`      | 读写分离配置：<br>`0` - 禁用 <br>`1` - 启用                                 |
| `other_property`  | `int`      | 用户角色标识：<br>`0` - 普通用户 <br>`1` - 统计用户 <br>`2` - 监控用户 <sup>*v2.4.7+*</sup><br>`3` - Admin用户 <sup>*v2.4.8+*</sup> |
