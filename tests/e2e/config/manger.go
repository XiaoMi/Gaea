...
var (
	DefaulNamespace = NewNamespace(
		WithIsEncrypt(true),
		WithName("test_namespace_default"),
		WithAllowedDBS(map[string]bool{
			"sql_DB":         true,
			"master_hint_DB": true,
		}),
		WithSlices(defaultNoShardSlice),
		WithUsers([]*models.User{
			{
				UserName:      "superroot",
				Password:      "superroot",
				Namespace:     "test_namespace_default",
				RWFlag:        2,
				RWSplit:       1,
				OtherProperty: 0,
			},
		}),
		WithDefaultSlice("slice-0"),
	)
)

var (
	defaultNoShardSlice = []*models.Slice{
		{
			Name:     "slice-0",
			UserName: "superroot",
			Password: "superroot",
	...
	defaultShardSlice = []*models.Slice{
		{
			Name:            "slice-0",
			UserName:        "superroot",
			Password:        "superroot",
			Master:          "127.0.0.1:3219",
			Slaves:          []string{},
			StatisticSlaves: []string{},
			Capacity:        12,
			MaxCapacity:     24,
