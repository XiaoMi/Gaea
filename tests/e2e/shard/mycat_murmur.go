...
var _ = ginkgo.Describe("shard join support test in mycat murmur", func() {
	...
	ginkgo.BeforeEach(func() {
		// 注册
		ns, err := config.ParseNamespaceTmpl(config.MycatMurmurNamespaceTmpl, sliceMulti)
	...
	ginkgo.Context("shard support test", func() {
		ginkgo.It("When testing sql support", func() {
			...
			cases := []struct {
				...
				{
					filepath.Join(e2eMgr.BasePath, "shard/case/join/mycat/murmur/equal.sql"),
					util.Equal,
					gaeaConn,
					singleMaster,
				},
				...
				{
					filepath.Join(e2eMgr.BasePath, "shard/case/join/mycat/murmur/show.sql"),
					util.Equal,
					gaeaConn,
					defaultShardConn,
				},
				{
					filepath.Join(e2eMgr.BasePath, "shard/case/join/mycat/murmur/unequal.sql"),
					util.UnEqual,
					gaeaConn,
					singleMaster,
				},
				{
					filepath.Join(e2eMgr.BasePath, "shard/case/join/mycat/murmur/unsupport.sql"),
					util.UnSupport,
					gaeaConn,
					singleMaster,
				},
