...
var _ = ginkgo.Describe("shard join support test in mycat long", func() {
	nsTemplateFile := "e2e/shard/ns/mycat/long.template"
	...
	ginkgo.Context("shard support test", func() {
		ginkgo.It("When testing sql support", func() {
			cases := []struct {
				...
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/long/equal.sql"),
					util.Equal,
				},
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/long/unequal.sql"),
					util.UnEqual,
				},
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/long/unsupport.sql"),
					util.UnSupport,
				},
			}
			for _, c := range cases {
