...
select sum(O_TOTALPRICE) as sums,O_CUSTKEY,count(O_ORDERKEY) counts from test9 where O_CUSTKEY between 'CUSTKEY_002' and 'CUSTKEY_050' group by 2 asc having O_CUSTKEY<(select max(c_custkey) from test7) order by count(O_ORDERKEY);
    ...
    ##select sum(O_TOTALPRICE) as sums,O_CUSTKEY,count(O_ORDERKEY) counts from test9 where O_CUSTKEY between 'CUSTKEY_002' and 'CUSTKEY_050' group by 2 asc having O_CUSTKEY<(select max(c_custkey) from test7) order by (select C_ORDERKEY from test7 where c_custkey=o_custkey) asc;
...
select id,O_CUSTKEY,O_ORDERKEY,O_TOTALPRICE from test9 a where (a.O_ORDERKEY,O_CUSTKEY)=(select c_ORDERKEY,c_CUSTKEY from test7 where c_name='yanglu');
    ...
    ##select id,O_CUSTKEY,O_ORDERKEY,O_TOTALPRICE from test9 a where (a.O_ORDERKEY,a.O_CUSTKEY) in (select c_ORDERKEY,c_CUSTKEY from test7) order by id,O_ORDERKEY,O_CUSTKEY,O_TOTALPRICE,MYDATE;
    ...
    ##select id,O_CUSTKEY,O_ORDERKEY,O_TOTALPRICE from test9 a where exists(select * from test7 where a.O_CUSTKEY=test7.C_CUSTKEY);
    ##select distinct O_ORDERKEY,O_CUSTKEY from test9 a where exists(select * from test7 where a.O_CUSTKEY=test7.C_CUSTKEY);
