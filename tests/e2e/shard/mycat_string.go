...
var _ = ginkgo.Describe("shard join support test in mycat string", func() {
	nsTemplateFile := "e2e/shard/ns/mycat/string.template"
	...
	ginkgo.Context("shard support test", func() {
		ginkgo.It("When testing sql support", func() {
			cases := []struct {
				...
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/string/equal.sql"),
					util.Equal,
				},
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/string/unequal.sql"),
					util.UnEqual,
				},
				{
					filepath.Join(e2eMgr.BasePath, "e2e/shard/case/join/mycat/string/unsupport.sql"),
					util.UnSupport,
				},
			}
			for _, c := range cases {
