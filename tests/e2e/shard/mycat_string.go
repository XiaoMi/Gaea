...
var _ = ginkgo.Describe("shard join support test in mycat string", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		// 3. 注册命名空间
		ns, err := config.ParseNamespaceTmpl(config.MycatStringNamespaceTmpl, sliceMulti)
		util.ExpectNoError(err)
		...
		_, err = gaeaConn.Exec("USE sbtest")
		util.ExpectNoError(err)
	...
	ginkgo.Context("shard support test", func() {
		ginkgo.It("When testing sql support", func() {
			...
			cases := []struct {
				...
				{
					filepath.Join(e2eMgr.BasePath, "shard/case/join/mycat/string/show.sql"),
					util.Equal,
					gaeaConn,
					defaultShardConn,
				},
